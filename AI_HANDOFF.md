...
4. 不要把 `C:\Users\Administrator\Desktop\web-main` 当成完整项目。当前建议工作目录是 `D:\web-main-clean`，旧目录 `D:\web-main` 只保留作备份。
...
- GitHub 主分支：`origin/main`
- 当前已发布最新提交：`943ff33 fix: align lightrag small embedding runtime`
- 本地仅未推送的小提交：`eddc131 chore: normalize text file line endings`
- 本地保险 tag：`backup/eddc131-20260505`
...
- `我的订单到哪了`：未登录应提示登录。
- `我的物流到哪了`：这是个人订单物流状态，应走订单实时接口；`物流一直不动怎么办` 才是物流规则知识库问题。
- 带伪造 token 的个人查询：应 401，不能把伪造 token 传给 AI 工具继续查。
- 售后、物流、优惠券规则问题：优先知识库，不应错误要求登录。

## 5. LightRAG / 知识库状态

当前目标是 LightRAG-only，不再继续依赖 Chroma。当前生产 embedding 主线已经切到 `BAAI/bge-small-zh-v1.5` / `512` 维。
...
- 开头会探测 LightRAG `/docs` 或 `/redoc`，除非传 `-SkipLightRagCheck`。
- 如果全部用例都是 `missing HTTP status code`，先启动/检查后端，不要改 AI 提示词。
- 如果商品/登录安全用例通过，只有售后/优惠券/物流失败，优先检查 LightRAG 运行态和知识库索引。
- 本地没有 LightRAG 时，知识库类用例失败是预期现象，不能据此说客服逻辑全坏。

内置回归用例目前重点覆盖：

- `苹果多少钱`（应返回 `route/sourceType=clarification`）
- `苹果15多少钱` / `苹果15Pro多少钱`
- `售后质量问题退回运费谁承担`
- `优惠券没到门槛为什么不能用`
- `物流一直不动怎么办`
- 伪造 token 访问个人查询应返回 `401`
...
当前 AI / embedding 运行基线：

- `evanshine-ai-service.service` 监听 `172.21.0.1:19000`
- `embedding-local.service` active
- `embedding-tunnel.service` active
...
- `EMBEDDING_DIM=512`
- LightRAG documents `processed=134`
...
- 当前服务器运行态已经验证通过，但服务器目录 `/opt/evanshine-shop/current` 不是 git repo，不能用 `git pull` 假设与 GitHub 完全同步。
...
1. 只读确认服务器状态。
2. 备份当前目录。
3. 如果后续需要部署，按当前 `origin/main` 或明确指定提交同步文件到 `/opt/evanshine-shop/current`。
...
```powershell
cd D:\web-main-clean\ai-service
python -m pytest
```
...
普通 `/chat` 和流式 `/chat/stream` 都应沉淀最终回复元数据。流式路径会从 SSE 的 `final.reply` 里解析 route/sourceType/hitLogs/fallbackReason，再写入 `customer_service_log`、`kb_hit_log` 或 `kb_miss_log`。
...
项目当前建议工作目录是 `D:\web-main-clean`。C 盘上的 `C:\Users\Administrator\Desktop\web-main` 不是完整仓库，旧的 `D:\web-main` 也不是当前建议开发目录。不要在 C 盘继续跑完整依赖下载。
...
```text
目前 GitHub `origin/main` 已到 `943ff33`，LightRAG-only AI 客服、商品实时查询、Token 鉴权、余额/优惠券/库存相关优化都已经在代码里。核云服务器当前运行态已经核过：小模型 embedding 主线、LightRAG 检索和客服 smoke 正常。后续如果需要部署新改动，我会先备份 `/opt/evanshine-shop/current`，只更新电商项目，不碰 Metapi，然后跑伪造 token、苹果宽词、售后/优惠券/物流知识库回归。
```
